...
interface IGeneratorConfig {
    data: string,
    batchSize: number,
    logKeyObjectWrapper: string, /* if included, data is wrapped in {"logKeyObjectWrapper": cell2} */
}

const defaultConfig: IGeneratorConfig = {
    data: "sample/sample.log",
    batchSize: 10,
    logKeyObjectWrapper: null,
};
        ...
        return {
            ...
            makeInstance: (() => (async function*() {
                ...
                for await (const line of rl) {
                    ...
                    batch.push((config.logKeyObjectWrapper) ?
                    {
                        [config.logKeyObjectWrapper]: cell2,
                    } :
                    parsed);
